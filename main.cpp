...
void print(Student &s)
{
    std::cout<<"排名"<<"\t"<<"学号"<<"\t"<<"姓名"<<"\t"<<"语文"<<"\t"<<"数学"<<"\t"<<"英语"<<"\t"<<"总分"<<"\t"<<"平均分"<<std::endl ;
    std::cout<<s.rank<<"\t"<<s.number<<"\t"<<s.name<<"\t"<<s.chinese<<"\t"<<s.math<<"\t"<<s.english<<"\t"<<s.total<<"\t"<<s.average<<std::endl ;
}
int search(Student S[],int N,char *n)
{
    for(int i=0;i<N;i++)
...
int main()
{
    ...
    while(order!=4)
    {
        std::cout<<"***************************命令菜单**********************"<<std::endl ;
        std::cout<<"1.打印所有排名"<<std::endl ;
        std::cout<<"2.打印出成绩在全班平均分以上的学生名单和数据信息"<<std::endl ;
        std::cout<<"3.任意输入一个学号，查找出该学生在班级中的排名及其成绩"<<std::endl ;
